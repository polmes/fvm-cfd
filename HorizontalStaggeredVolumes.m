	...
	methods
		function this = HorizontalStaggeredVolumes(MSH)
			for el=1:size(MSH.REL,2)
				this.vidx(:,el)=MSH.IDX(2,[MSH.REL(1,el),%1
										   MSH.REL(1,MSH.REL(2,el)),%2
										   MSH.REL(3,el),%3
										   el]);%4
...
this.uidx
		...
		function Kc = calcKc(this,v,MSH)
			...
			for el=1:size(this.uidx,2)
				...
				Kc(this.uidx(1,el),nu(1))=Fn+Fe-Fs-Fw;
				Kc(this.uidx(1,el),nu(2:end))=[Fn Fe -Fs -Fw];
			end
		end

		function Kd = calcKd(this,v,MSH)
